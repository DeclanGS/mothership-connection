...
skills = {'Linguistics', 'Zoology', 'Botany', 'Geology', 'Industrial Equipment', 'Jury-Rigging', 'Chemistry',
          'Computers', 'Zero-G', 'Mathematics', 'Art', 'Archaeology', 'Theology', 'Military Training', 'Rimwise', 'Athletics'}

char = {'class' : choice(['Marine','Android','Scientist','Teamster'])}
             
char['strength'] = randrange(1,10) + randrange(1,10) + 20
char['speed'] = randrange(1,10) + randrange(1,10) + 20
char['intellect'] = randrange(1,10) + randrange(1,10) + 20
char['combat'] = randrange(1,10) + randrange(1,10) + 20

char['sanity'] = randrange(1,10) + randrange(1,10) + 10
char['fear'] = randrange(1,10) + randrange(1,10) + 10
char['body'] = randrange(1,10) + randrange(1,10) + 10

char['wounds'] = 2
char['health'] = randrange(1,10) + 10
char['stress'] = 2

if char['class'] == 'Marine':
    char['combat'] += 10
    char['body'] += 10
    char['fear'] += 20
    char['wounds'] = 3
...
if char['class'] == 'Android':
    char['intellect'] += 20
    char['fear'] += 60
    char['wounds'] = 3
    # mod_stat = input('You are an Android. Select a stat to penalize.').lower()
    mod_stat = choice(['strength','speed','intellect','combat'])
    char[mod_stat] -= 10
...
if char['class'] == 'Scientist':
    char['intellect'] += 10
    char['sanity'] += 30
    # mod_stat = input('You are a Scientist. Select a stat to augment.').lower()
    mod_stat = choice(['strength','speed','intellect','combat'])
    char[mod_stat] += 5

    doctorate = choice(['Sophontology', 'Exobiology', 'Surgery', 'Planetology', 'Robotics', 'Engineering',
                        'Cybernetics', 'Artificial Intelligence', 'Hyperspace', 'Xenoesotericism'])
    
    masters = choice({'Sophontology': ['Psychology'], 'Exobiology': ['Pathology'], 'Surgery': ['Pathology', 'Field Medicine'],
    ...
    bach = choice({'Psychology': ['Zoology'], 'Pathology': ['Zoology', 'Botany'], 'Field Medicine': ['Zoology', 'Botany'],
    ...
    char['skills'] = bach + ', ' + masters + ', and ' + doctorate + '; and also ' + skills.pop()
...
if char['class'] == 'Teamster':
    ...
    char['fear'] += 10
    char['body'] += 10
    skills -= {'Industrial Equipment', 'Zero-G'}

    jobSkill = skills.pop()

    jobAdvanced = {'Linguistics': [], 'Zoology': ['Psychology', 'Pathology', 'Field Medicine'],
    ...
    char['skills'] = 'Industrial Equipment, Zero-G, ' + jobSkill + ', and ' + choice(['Asteroid Mining', 'Mechanical Repair', 'Piloting'] + jobAdvanced)
...
char['trinket'] = choice(['Manual: PANIC: Harbinger of Catastrophe',
                ...
                'Dessicated Husk Doll', 'Key (Childhood Home)', 'Gyroscope, Bent, Tin',
                'Alien Pressed Flower (common)', 'Dog Tags (Heirloom)', 'Faded Green Poker Chip',
                'Necklace of Shell Casings', 'Token: “Is Your Morale Improving?”', 'Ukulele',
                'Corroded Android Logic Core', 'Phosphorescent Sticks, Neon', 'Spray Paint',
                'Pamphlet: Signs of Parasitical Infection', 'Pamphlet: The Indifferent Stars', 'Wanted Poster, Weathered',
                'Manual: Treat Your Rifle Like A Lady', 'Calendar: Military Battles', 'Locket, Hair Braid',
                'Bone Knife', 'Manual: Rich Captain, Poor Captain', 'Scultpure of a Rat (Gold)',
                ...
                'Holographic Serpentine Dancer', 'Titanium Toothpick', 'Gun',
                'Snake Whiskey', 'Gloves, Leather (Xenomorph Hide)', 'Flint Hatchet',
                'Medical Container, Purple Powder', 'Smut (Seditious): The Captain, Ordered', 'Pendant: Two Astronauts form a Skull',
                'Pills: Male Enhancement, Shoddy', 'Towel, Slightly Frayed', 'Rubik\'s Cube',
                'Casino Playing Cards', 'Brass Knuckles', 'Stress Ball, reads: Zero Stress in Zero G',
                'Lagomorph Foot', 'Fuzzy Handcuffs', 'Sputnik Pin',
                'Moonstone Ring', 'Journal of Grudges', 'Ushanka',
                'Manual: Mining Safety and You', 'Stylized Cigarette Case', 'Trucker Cap, Mesh, Grey Alien Logo',
                'Pamphlet: Against Human Simulacrum', 'Ball of Assorted Gauge Wire', 'Menthol Balm',
                'Animal Skull, 3 Eyes, Curled Horns', 'Spanner', 'Pith Helmet',
                'Bartender’s Certification (Expired)', 'Switchblade, Ornamental', '10m x10m Tarp',
                'Bent Wrench', 'Powdered Xenomorph Horn', 'I Ching, Missing Sticks',
                'Prospecting Mug, Dented', 'Bonsai Tree, Potted', 'Kukri',
                'Eerie Mask', 'Golf Club (Putter)', 'Trench Shovel',
                'Ultrablack Marble', 'Trilobite Fossil', 'Shiv, Sharpened Butter Knife',
                'Ivory Dice', 'Pamphlet: A Lover In Every Port', 'Taxidermied Cat',
                ...
                'Ashes (A Relative)', 'Spiked Bracelet', 'Opera Glasses',
                'DNR Beacon Necklace', 'Harmonica', 'Pamphlet: Android Overlords',
                'Cigarettes (Grinning Skull)', 'Pictorial Pornography, Dogeared, Well-thumbed', 'Pamphlet: The Relic of Flesh',
                'Pills: Areca Nut'])
